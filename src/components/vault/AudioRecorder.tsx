    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          const resp = await fetch("/api/vault/transcribe", { method: "POST", body: fd });
          const data = await resp.json();
          if (!resp.ok) { setError(data.error || "Transcription failed"); return; }
          onTranscription({ transcript: data.transcript, language: data.language, structured: data.structured, confidence: data.confidence });
        } catch { setError("Failed to process audio."); } finally { setProcessing(false); }
      };
    ...
    <div className="space-y-3">
      ...
      {processing ? (
        <div className="flex items-center gap-3 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4">
          ...
          <div>
            <p className="text-sm font-medium text-stone-700 dark:text-stone-300">Processing recording...</p>
            <p className="text-xs text-stone-500 dark:text-stone-400">Transcribing + structuring (10-20s)</p>
          </div>
        </div>
      ) : recording ? (
        <div className="flex items-center gap-4">
          <button onClick={stopRecording} className="flex h-14 w-14 items-center justify-center rounded-full bg-red-500 text-white shadow-lg hover:bg-red-600 animate-pulse">
            <Square className="h-6 w-6" />
          </button>
          <div>
            <p className="text-sm font-medium text-red-600 dark:text-red-400">Recording... {fmt(duration)}</p>
            <p className="text-xs text-stone-500 dark:text-stone-400">Speak your recipe. Tap to stop.</p>
          </div>
        </div>
      ) : (
        <button onClick={startRecording} disabled={disabled || !title.trim()}
          ...
          <div>
            ...
            <p className="text-xs text-stone-500 dark:text-stone-400">Speak in any language — 99+ supported</p>
          </div>
        </button>
      )}
    </div>
